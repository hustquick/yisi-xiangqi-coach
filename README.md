...
本项目据此采用“棋盘优先”的信息层级：竖向或窄屏时棋盘优先、极简工具在上、折叠面板在下；横向或宽屏时棋盘与极简工具在左、折叠面板在右。极简工具统一为 `↶`、`↷`、`优`、`⇅`、`↻`；双人对弈、人机对战、摆盘工具、电脑等级和分析参数合并到“对弈与分析设置”折叠模块，摆盘不会在棋盘附近新增面板；棋谱与存档排在辅助模块末端且默认折叠。折叠不会清除当前局面或配置，用户可随时恢复这些功能。
...
- 人机对战提供“业余一级”到“专业九级”十档参考强度，并通过皮卡鱼的 `UCI_LimitStrength` 与 `UCI_Elo` 直接选择电脑应着
...
- npm
- 首次安装原生 Pikafish 时需要 C++ 编译工具、Make 和网络连接
...
`local` 命令会在 `127.0.0.1:8789` 启动多线程 Pikafish 服务，并启动网页开发服务器。页面优先使用原生引擎；原生服务不可用时，会尝试随网页提供的 WASM 引擎。该端口与国际象棋的 Stockfish 分离，两个项目可同时运行。
...
检查是否已经启动了另一个 `npm run local` 实例。HTML 开发服务器使用端口 `3000`（已占用时 Vite 会选用下一个可用端口），本地 Pikafish 服务使用 `127.0.0.1:8789`。
