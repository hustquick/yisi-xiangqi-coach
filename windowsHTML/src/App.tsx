...
function sideAfterPly(start: Side, ply: number): Side { return ply % 2 === 0 ? start : start === "red" ? "black" : "red"; }
...
export default function Home() {
  ...
  useEffect(() => {
    const worker = createPikafishWorker();
  ...
  useEffect(() => {
    ...
    workerRef.current.postMessage({
      ...
      // Historical scores are deliberately lower priority than the live board.
      // A shallow backfill keeps the single-threaded WASM worker available for
      // the user's next move as quickly as possible.
      depth: Math.min(6, analysisDepth),
      multiPV: 1,
    });
  }, [
    engineConnected,
  ...
  useEffect(() => {
    ...
    // The analysis result already belongs to the current position. Apply the
    // engine move immediately so a render unrelated to the board cannot cancel
    // a delayed timer and leave the computer stuck on "thinking".
  ...
  function reset() {
    ...
    setStartingPieces(initialPieces); setStartingTurn("red"); setRecordTitle("新对局"); setRecordMessage("");
    setHistory([]);
    setPositionScores({});
    setActivePly(0);
    setTurn("red");
  ...
  function installRecord(imported: { title: string; pieces: Piece[]; turn: Side; moves: Array<{ from: [number, number]; to: [number, number] }> }, scores: Record<number, number> = {}, requestedPly?: number) {
    ...
    for (const coordinates of imported.moves) {
      ...
      if (!moving) throw new Error(`第 ${nextHistory.length + 1} 步的起点没有棋子。`);
      const captured = position.find((piece) => piece.x === coordinates.to[0] && piece.y === coordinates.to[1]);
      nextHistory.push({ piece: { ...moving }, from: coordinates.from, to: coordinates.to, captured: captured ? { ...captured } : undefined });
      position = position.filter((piece) => piece.id !== captured?.id).map((piece) => piece.id === moving.id ? { ...piece, x: coordinates.to[0], y: coordinates.to[1] } : piece);
    }
    const target = Math.max(0, Math.min(requestedPly ?? nextHistory.length, nextHistory.length));
  ...
  async function importRecordFile(file: File) {
    try {
      ...
      else if (/\.json$/i.test(file.name)) { const saved = JSON.parse(await file.text()); if (saved.version !== SAVED_GAME_VERSION) throw new Error("不支持此存档版本。"); installRecord(saved.record, saved.positionScores || {}, saved.activePly); }
      else installRecord(parseFen(await file.text()) as Parameters<typeof installRecord>[0]);
    } catch (error) { setRecordMessage(error instanceof Error ? error.message : "棋谱载入失败。"); }
    if (recordFileRef.current) recordFileRef.current.value = "";
  }

  function saveGame() {
    const record = { title: recordTitle === "新对局" ? `象棋对局 ${new Date().toLocaleString("zh-CN")}` : recordTitle, pieces: startingPieces, turn: startingTurn, moves: history.map((move) => ({ from: move.from, to: move.to })) };
    localStorage.setItem("yisi-xiangqi-windows-save", JSON.stringify({ version: SAVED_GAME_VERSION, record, activePly, positionScores })); setRecordTitle(record.title); setRecordMessage(`已保存“${record.title}”。`);
  }

  function loadSavedGame() {
    try { const saved = JSON.parse(localStorage.getItem("yisi-xiangqi-windows-save") || "null"); if (!saved) throw new Error("还没有本机存档。"); installRecord(saved.record, saved.positionScores || {}, saved.activePly); }
    ...
    <main>
      ...
      <Collapsible title="棋谱与存档"><section className="record-toolbar panel">
        ...
        <div className="record-stepper">
          <button onClick={() => goToPly(0)} disabled={!activePly}><i>⇤</i><span>开始</span></button><button onClick={() => goToPly(activePly - 1)} disabled={!activePly}><i>‹</i><span>上一步</span></button>
          <button onClick={() => goToPly(activePly + 1)} disabled={activePly === history.length}><span>下一步</span><i>›</i></button><button onClick={() => goToPly(history.length)} disabled={activePly === history.length}><span>末尾</span><i>⇥</i></button>
        </div>
      ...
      {showRecordPanel && <section className="record-loader panel">
        <div className="record-loader-heading">
          <div><span>资料库</span><strong>载入棋谱或局面</strong><small>所有文件均在本机离线读取，不会上传</small></div>
          <button className="record-loader-close" onClick={() => setShowRecordPanel(false)} aria-label="关闭载入面板">×</button>
        </div>
        <div className="record-loader-grid">
          <button className="record-file-card" onClick={() => recordFileRef.current?.click()}>
            <i aria-hidden="true">↥</i><span><strong>选择本地文件</strong><small>XQF 1.0 · FEN · 弈思 JSON</small></span><b>选择</b>
          </button>
          <div className="record-fen-card">
            <label htmlFor="windows-fen-input"><i>局</i><span><strong>粘贴 FEN 局面</strong><small>输入完整的十行象棋局面代码</small></span></label>
            <div><textarea id="windows-fen-input" value={fenInput} onChange={(event) => setFenInput(event.target.value)} placeholder="例如：rnbakabnr/9/1c5c1/... w" /><button onClick={() => { try { installRecord(parseFen(fenInput) as Parameters<typeof installRecord>[0]); } catch (error) { setRecordMessage(error instanceof Error ? error.message : "FEN 载入失败。"); } }} disabled={!fenInput.trim()}>载入局面</button></div>
          </div>
          <button className="record-local-card" onClick={loadSavedGame}>
            <i aria-hidden="true">⌑</i><span><strong>本机存档</strong><small>继续上次保存的棋局</small></span><b>载入</b>
          </button>
        </div>
      </section>}
      ...
      <section className="workspace">
        <div className="board-wrap" ref={boardSectionRef}>
          <div className="board-top">
            ...
            <button className="perspective-button" onClick={() => setBoardFlipped((value) => !value)} aria-label="切换红黑视角">⇅ 视角</button>
            <button
      ...
      <footer>
        ...
        <a className="engine-credit" href="./PIKAFISH-SOURCE.txt" target="_blank">
          Pikafish · GPL-3.0
        </a>
      </footer>
